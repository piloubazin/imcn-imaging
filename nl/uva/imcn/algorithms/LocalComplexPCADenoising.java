            ...
            if (process) {
               ...
               for (int n=0;n<ngbN;n++) {
                ...
                if (randomMatrix) {
                    ...
                    // 1. renormalize to eigenvalues
                    double[] eignorm = new double[ngbN];
                    for (int n=0;n<ngbN;n++) {
                        eignorm[n] = eig[n]*eig[n]/ndim;
                    }
                    // 2. increase the number of used eigenvalues
                    int nkept = ngbN-1;
         
                    for (int n=0;n<ngbN-1;n++) {
                        double eignormsum = 0.0;
                        for (int m=n+1;m<ngbN;m++) {
                            eignormsum += eignorm[m];
                        }
                        double sigma = (eignorm[n+1]-eignorm[ngbN-1])/(4.0*FastMath.sqrt(ngbN-n)/(double)ndim);
                        if (eignormsum> (ngbN-n)*sigma) {
                            nkept = n;
                            n = ngbN;
                        }
                    }
