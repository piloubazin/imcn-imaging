...
public class LinearFiberMapping {
    ...
    private final void estimateRadius(float[] image, boolean[] object, int[] scaleMax, byte[] finalDirection, boolean[] maskCort) {
		...
		boolean[] maskOut=new boolean[nxy];
		boolean[] maskVote=new boolean[nxy];
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) {
			int id = x + nx*y;
			maskOut[id]=false;
						...
						while(dist<=rOut){	
							...
							if(x-s*orthVector[X]>=0 && x+s*orthVector[X]<nx && (y-s*orthVector[Y])>=0 && (y-s*orthVector[Y])<ny){
								if(dist>rIn){
									...
									if(!object[x-s*orthVector[X]+nx*(y-s*orthVector[Y])+nx*ny*(z-s*orthVector[X])])	maskVote[x-s*orthVector[X]+nx*(y-s*orthVector[Y])]=true;
								}
							}
							s++;
		...
		boolean[][] obj2=new boolean[nxy][5]; 
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) {
			int id = x + nx*y;
			obj2[id][0]=false;
			if(object[id]){
